...
{  
    ...
    double x0, y0, u_x, u_y, v_x, v_y, d1_x, d1_y, d2_x, d2_y; 
    ...
    for(i=2; i < IT-2; i++)
    {
        ...
        for(j=2; j < JT-2; j++)
        {
            x0 = 0.5 * (x[IN(i,j)] + x[IN(i,j+1)]);
            y0 = 0.5 * (y[IN(i,j)] + y[IN(i,j+1)]);
            u_x = x0 - x[IN(i,2)];
            u_y = y0 - y[IN(i,2)];
            v_x = x[IN(i+1,2)] - x[IN(i,2)];
            v_y = y[IN(i+1,2)] - y[IN(i,2)];
            d1_x = u_x - v_x;
            d1_y = u_y - v_y;
            d2_x = u_x + v_x;
            d2_y = u_y + v_x;

            y_ = 0.5 * sqrt((pow(d1_x,2) + pow(d1_y,2)) * (pow(d2_x,2) + pow(d2_y,2)) / (pow(v_x,2) + pow(v_x,2)));
            ...
            omega = fabs(dxflr[2][Ilr(i,j)] - dyflr[1][Ilr(i,j)]);
            // omega = sqrt(pow(dxflr[2][Ilr(i,j)], 2) + pow(dyflr[1][Ilr(i,j)], 2));
...
{  
    ...
    for(i=2; i < IT-2; i++)
    {
        ...
        for(j=2; j < JT-2; j++)
        {
            x0 = 0.5 * (x[IN(i,j)] + x[IN(i+1,j)]);
            y0 = 0.5 * (y[IN(i,j)] + y[IN(i+1,j)]);
            u_x = x0 - x[IN(i,2)];
            u_y = y0 - y[IN(i,2)];
            v_x = x[IN(i+1,2)] - x[IN(i,2)];
            v_y = y[IN(i+1,2)] - y[IN(i,2)];
            ...
            omega = fabs(dxfdu[2][Idu(i,j)] - dyfdu[1][Idu(i,j)]);
            // omega = sqrt(pow(dxfdu[2][Idu(i,j)], 2) + pow(dyfdu[1][Idu(i,j)], 2));
            // printf("%e\n", omega);
            F_y = y_ * fabs(omega) * (1 - exp((-y_ * nu_) / (A * nu))); // F(y)
