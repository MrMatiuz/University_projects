...
// ======================================
double *mu_turb_in_lr, *mu_turb_in_du;
double *mu_turb_out_lr, *mu_turb_out_du;
double *Y_max_lr, *Y_max_du;
// ======================================
int Flag_turb;
...
double* get_intersection(double a[], double b[], int arr_len, int needed_intersection_num)
{
    double c[arr_len];
    ...
    // needed_intersection_num = min(needed_intersection_num, sizeof(intersection) / sizeof(intersection[0]));
...
{  
    ...
    double S;
    double local_mu_turb_in[JT-4];
    double local_mu_turb_out[JT-4];
    ...
    for(i=2; i < IT-2; i++)
    {
        ...
        for(j=2; j < JT-2; j++)
        {
            ...
            mu_turb_in_lr[Ilr(i,j)] =  mu_Ti * gdf_0;
            mu_turb_out_lr[Ilr(i,j)] =  mu_To * gdf_0;
            Y_max_lr[Ilr(i,j)] = Y_max;

            local_mu_turb_in[j-2] = mu_Ti * gdf_0;
            local_mu_turb_out[j-2] = mu_To * gdf_0;
...
// ==============================================================================================================
        int needed_intersection_num = 3;
        double *r;

        r = get_intersection(local_mu_turb_out, local_mu_turb_in, JT-2, needed_intersection_num);

        for(j=2; j < JT-2; j++)
            mu_turb_lr[Ilr(i,j)] = r[j-2];

        // for(j=2; j < JT-2; j++)
        //     printf("[%f; %f] ", local_mu_turb_out[j-2], local_mu_turb_in[j-2]);
        // printf("\n");
// ==============================================================================================================
...
{  
    ...
    for(i=2; i < IT-2; i++)
    {
        ...
        for(j=2; j < JT-2; j++)
        {
            ...
            mu_turb_in_du[Idu(i,j)] =  mu_Ti * gdf_0;
            mu_turb_out_du[Idu(i,j)] =  mu_To * gdf_0;
            Y_max_du[Idu(i,j)] = Y_max;
