...
// ======================================
extern double *mu_turb_in_lr, *mu_turb_in_du;
extern double *mu_turb_out_lr, *mu_turb_out_du;
extern double *Y_max_lr, *Y_max_du;
// ======================================
...
void out_tec(void)
{
 ...
 fprintf(fpw," VARIABLES = \"X\", \"Y\", \"D\", \"U\", \"V\", \"P\",\"MU_TURB\",\"MU_TURB_IN\",\"MU_TURB_OUT\",\"Y_max\"  \n");
 fprintf(fpw," ZONE F=BLOCK  I=%d  J=%d \n", JT-3, IT-3);
 fprintf(fpw," VARLOCATION=([3-10]=CELLCENTERED) \n");
 ...
 for(i=2; i < IT-2; i++)
  for(j=2; j < JT-2; j++)
   {
    fprintf(fpw," %12.4e ", 0.25 * (mu_turb_in_lr[Ilr(i,j)] + mu_turb_in_lr[Ilr(i-1,j)] + mu_turb_in_du[Idu(i,j)] + mu_turb_in_du[Idu(i,j-1)])); // mu_turb_in_arr[IC(i,j)]
    iprn++;
    if(iprn > 6) { iprn=1; fprintf(fpw,"\n"); }
  }
 fprintf(fpw,"\n");
 ...
 for(i=2; i < IT-2; i++)
  for(j=2; j < JT-2; j++)
   {
    fprintf(fpw," %12.4e ", 0.25 * (mu_turb_out_lr[Ilr(i,j)] + mu_turb_out_lr[Ilr(i-1,j)] + mu_turb_out_du[Idu(i,j)] + mu_turb_out_du[Idu(i,j-1)])); // mu_turb_out_arr[IC(i,j)]
    iprn++;
    if(iprn > 6) { iprn=1; fprintf(fpw,"\n"); }
  }
 fprintf(fpw,"\n");
 ...
 for(i=2; i < IT-2; i++)
  for(j=2; j < JT-2; j++)
   {
    fprintf(fpw," %12.4e ", 0.25 * (Y_max_lr[Ilr(i,j)] + Y_max_lr[Ilr(i-1,j)] + Y_max_du[Idu(i,j)] + Y_max_du[Idu(i,j-1)])); //Y_max_arr[IC(i,j)]
    iprn++;
    if(iprn > 6) { iprn=1; fprintf(fpw,"\n"); }
  }
 fprintf(fpw,"\n");
...
// ======================================
 mu_turb_in_lr=(double*)malloc(len*sizeof(double));
 mu_turb_in_du=(double*)malloc(len*sizeof(double));
 mu_turb_out_lr=(double*)malloc(len*sizeof(double));
 mu_turb_out_du=(double*)malloc(len*sizeof(double));
 Y_max_lr=(double*)malloc(len*sizeof(double));
 Y_max_du=(double*)malloc(len*sizeof(double));;
// ======================================
