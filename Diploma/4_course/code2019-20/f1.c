...
double *mu_turb_lr, *mu_turb_du, *mu_To_du, *mu_Ti_du, *mu_To_lr, *mu_Ti_lr;
int Flag_turb;
...
{
 fscanf(fpr,"%lg",&Radj);
 fscanf(fpr,"%lg  %lg  %lg  %lg",&Dj,&Uj,&Vj,&Pj);
}
...
/*
   for(j=0; j < JT; j++)
   {
    ...
    gdf[2][IC(0,j)]=gdf[2][IC(1,j)]=-gdf[2][IC(2,j)];
   }
*/
 ...
 double mum,c,gdb[6];
   ...
   for(i=2; i <= IT-2; i++)
    for(j=2; j < JT-2; j++)
     ...
     for(k=0; k < 5; k++)
      {
//      df1=gdf[k][IC(i+1,j)]-gdf[k][IC(i,j)];
      df1=gdf[k][IC(i,j)]-gdf[k][IC(i-1,j)];
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!
/*
      f3=0.25*(gdf[k][IC(i,j-1)]+gdf[k][IC(i+1,j-1)]+                                                           f3=0.25*(gdf[k][IC(i,j-1)]+gdf[k][IC(i+1,j-1)]+
               gdf[k][IC(i+1,j)]+gdf[k][IC(i,j)]); 
      f4=0.25*(gdf[k][IC(i,j)]+gdf[k][IC(i+1,j)]+
 ...
 double mum,c,gdb[6];
   ...
   for(i=2; i < IT-2; i++)
    for(j=2; j <= JT-2; j++)
     ...
     for(k=0; k < 5; k++)
      {
//      df2=gdf[k][IC(i,j+1)]-gdf[k][IC(i,j)];
...
*/
		delf1 = (gdf[k][IC(i-1,j)] - gdf[k][IC(i-2,j)]);
		delf2 = (gdf[k][IC(i,j)] - gdf[k][IC(i-1,j)]);
                s=VA(delf1,delf2);  
                ff[k]=gdf[k][IC(i-1,j)]+0.25*s*((1.-ak*s)*delf1+(1.+ak*s)*delf2);
	}
...
*/
	for( k=0; k < 5; k++)
	{
		delf1=(gdf[k][IC(i,j)]-gdf[k][IC(i-1,j)]);
		delf2=(gdf[k][IC(i+1,j)]-gdf[k][IC(i,j)]);
		s=VA(delf1,delf2);
		ff[k]=gdf[k][IC(i,j)]-0.25*s*((1.-ak*s)*delf2+(1.+ak*s)*delf1);
	}
...
// NS_flux
// !!!!!!!!!!!!!!!!!!!!
     // mu_mol_du[Ilr(i,j)]=pow((pb/db),0.76);
     // muef=mu_ref*pow((pb/db),0.76);
     if(Flag_turb == 1){
      // mu_turb_arr[Ilr(i,j)] = 0.5 * (mu_turb_lr[Ilr(i,j)] + mu_turb_lr[Ilr(i+1,j)]);
      // mu_mol_arr[Ilr(i,j)] = 0.5 * (mu_mol_lr[Ilr(i,j)] + mu_mol_lr[Ilr(i+1,j)]);
      ...
      muef=mu_ref*pow((pb/db),0.76);
      // mu_mol_arr[Ilr(i,j)] = 0.5 * (mu_mol_lr[Ilr(i,j)] + mu_mol_lr[Ilr(i+1,j)]);
      // mu_lr[Ilr(i,j)] = mu_ref * pow((pb/db),0.76);
     }
...
*/
	for( k=0; k < 5; k++)
	{
		delf1=(gdf[k][IC(i,j-1)]-gdf[k][IC(i,j-2)]);
		delf2=(gdf[k][IC(i,j)]-gdf[k][IC(i,j-1)]);
		s=VA(delf1,delf2);
		ff[k]=gdf[k][IC(i,j-1)]+0.25*s*((1.-ak*s)*delf1+(1.+ak*s)*delf2);
	}
...
*/
	for( k=0; k < 5; k++)
	{
		delf1=(gdf[k][IC(i,j)]-gdf[k][IC(i,j-1)]);
		delf2=(gdf[k][IC(i,j+1)]-gdf[k][IC(i,j)]);
		s=VA(delf1,delf2);
		ff[k]=gdf[k][IC(i,j)]-0.25*s*((1.-ak*s)*delf2+(1.+ak*s)*delf1);
	}
...
//   !!!!!!!!!!!!!!!!!!!!
     // muef=mu_ref*pow((pb/db),0.76);
     // mu_mol_du[Idu(i,j)]=pow((pb/db),0.76);
     if(Flag_turb == 1){
      // mu_turb_arr[IC(i,j)] = 0.5 * mu_ref * (mu_turb_arr[Ilr(i,j)] + 0.5 * (mu_turb_du[Idu(i,j)] + mu_turb_du[Idu(i,j+1)]));
      // mu_mol_arr[IC(i,j)] = 0.5 * mu_ref * (mu_mol_arr[Ilr(i,j)] + 0.5 * (mu_mol_du[Idu(i,j)] + mu_mol_du[Idu(i,j+1)]));


      muef = mu_ref * pow((pb/db),0.76) + mu_turb_du[Idu(i,j)];
      // mu_du[Idu(i,j)] = mu_ref * pow((pb/db),0.76);
...
{  
 ...
 double eps = 1.e-4;
 double mu_To, mu_Ti;
 double k = 0.4, k_caps = 0.0168, C_cp = 1.6, C_kleb = 0.3, C_wk = 0.25, A = 26;
 ...
 for(i=2; i < IT-2; i++){
  ...
  for(j=2; j < JT-2; j++){
   ...
   // mu_Ti = k * y_ * D * fabs(omega); // Turb in
   mu_To = k_caps * C_cp * F_wake * Gamma; // Turb out
   
   mu_Ti_lr[Ilr(i,j)] = mu_Ti * gdf_0;
   mu_To_lr[Ilr(i,j)] = mu_To * gdf_0;

   // mu_To_max = max(mu_To_max, mu_To);
   // mu_Ti_max = max(mu_Ti_max, mu_Ti);
   // bug_min = mix(bug_min, dyf[1][IC(i,j)]);
   
   //Отбор вязкости
   if(fabs(mu_To - mu_Ti) < eps && Flag == 0){
    Flag = 1;
    num++;
    // printf("|");
   }
   mu_turb_lr[Ilr(i,j)] = (Flag == 1 ) ? mu_To : mu_Ti;
   mu_turb_lr[Ilr(i,j)] = mu_turb_lr[Ilr(i,j)] * gdf_0;
   // fprintf(fp,"%e ", mu_turb);
  }
  // printf("%e  ", bug_min);
  // printf(" mu_To=%e  mu_Ti=%e\n", mu_To_max, mu_Ti_max);
  // fprintf(fp,"\n");
  F_max = 0;
  Y_max = 0;
  U_max = 0;
...
{  
 ...
 double U_dif;
 double nu, omega, mu_m;
 double mu_To_max = 100, mu_Ti_max = 100, bug_min = 9.9e+9;
 double gdf_0, gdf_1, gdf_3, y_;
 int max_i, max_j;
 ...
 for(i=2; i < IT-2; i++){
  ...
  for(j=2; j < JT-2; j++){
   ...
   mu_Ti_du[Idu(i,j)] = mu_Ti * gdf_0;
   mu_To_du[Idu(i,j)] = mu_To * gdf_0;
   ...
   // mu_To_max = min(mu_To_max, mu_To);
   // mu_Ti_max = min(mu_Ti_max, mu_Ti);
   // printf(" mu_To=%e  mu_Ti=%e  i=%d   j=%d\n", mu_To, mu_Ti, i, j);
   // bug_min = mix(bug_min, dyf[1][IC(i,j)]);
   ...
   mu_turb_du[Idu(i,j)] = (Flag == 1 ) ? mu_To : mu_Ti;
   mu_turb_du[Idu(i,j)] = mu_turb_du[Idu(i,j)] * gdf_0;
   // fprintf(fp,"%e ", mu_turb);
  }
  // printf("%e  ", bug_min);
  // printf(" mu_To_max=%e  mu_Ti_max=%e  max_j=%d\n", mu_To_max, mu_Ti_max, max_j);
  // fprintf(fp,"\n");
  F_max = 0;
  Y_max = 0;
  U_max = 0;
