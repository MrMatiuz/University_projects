...
int n_out, i_app,i_flow,i_qgd,n_stp,max_stp,inp_par,IT,JT;
...
int  main(void)
{
 ...
 fscanf(fpr,"%lg",&t_max);
 fscanf(fpr,"%d %d %d",&max_stp,&n_out,&inp_par);
 fscanf(fpr,"%d  %d  %d",&i_app,&i_flow,&i_qgd);
...
mum=mu_ref*pow((gdf[3][n]/gdf[0][n]),0.76);
c=sqrt(GAM*gdf[3][n]/gdf[0][n]);
if(i_qgd) tau=mum/gdf[3][n];
else tau=0.;
...
// left  values for riem
	...
	for( k=0; k < 5; k++)
	{
		...
		frul[k]=ff[k]=gdf[k][IC(i-1,j)]+0.5*dksi2*delf;
	}
...
// right values for riem
	...
	for( k=0; k < 5; k++)
	{
		...
		frur[k]=ff[k]=gdf[k][IC(i,j)]-0.5*dksi1*delf;
	}
...
{
fllrRusanov(i,j,frul,frur,db,ub,vb,pb,par );
s1 =par[9];   s3 =par[11];
}
// NS_flux
...
// left(down)  values for riem
	...
	for( k=0; k < 5; k++)
	{
		...
		frul[k]=ff[k]=gdf[k][IC(i,j-1)]+0.5*deta2*delf;
	}
...
// right (up) values for riem
	...
	for( k=0; k < 4; k++)
	{
		...
		frur[k]=ff[k]=gdf[k][IC(i,j)]-0.5*deta1*delf;
	}
...
{
flduRusanov( i, j,frul, frur, db,ub,vb,pb, par);
s1 =par[9];   s3 =par[11];
}
// NS_flux
     muef=mu_ref*pow((pb/db),0.76);
...
void new_gdf(void)
{
 int i,j,n,k;
 double p,e;
 double rhs[5],a0[5],a1[5],a_[5],fl[5];
...
if(coord > 0.)
{
    ...
    kapef=3.5*muef/0.72;
    t11=2.*muef*(2.*dxf[1][n]-dyf[2][n]-gdf[2][n]/yc[n])/3.;
    t12=muef*(dyf[1][n]+dxf[2][n]);
    t21=t12;
    t22=2.*muef*(2.*dyf[2][n]-dxf[1][n]-gdf[2][n]/yc[n])/3.;
    t33=2.*muef*(-dxf[1][n]-dyf[2][n]+2.*gdf[2][n]/yc[n])/3.;
    q1=kapef*dxf[4][n];
    q2=kapef*dyf[4][n];
...
//     a1[k]=a1[k]/(1.+tau/dt);
  ...
  {
     a1[k]=a0[k]-(fllr[k][Ilr(i+1,j)]*fact-fllr[k][Ilr(i,j)]*fact+fldu[k][Idu(i,j+1)]*fact-fldu[k][Idu(i,j)]*fact)+dt*rhs[k];
  }
     gdf_[k][n]=gdf[k][n];
     }
    ...
    gdf[3][n]=p;
if(gdf[0][n] < 1.e-15 || gdf[3][n] < 1.e-15)
{
printf("ATT from new_gdf i=%d  j=%d  D=%e  P=%e \n",i,j,gdf[0][n],gdf[3][n]);
for(k=0; k < 4; k++)
printf("Fluxes: %e  %e  %e  %e \n",fllr[k][Ilr(i+1,j)],fllr[k][Ilr(i,j)], 
                                   fldu[k][Idu(i,j+1)],fldu[k][Idu(i,j)]);
printf("tau=%e  lx=%e  ly=%e mu_ref=%e  \n",tau,lxmin,lymin,mu_ref);
exit(-1);
