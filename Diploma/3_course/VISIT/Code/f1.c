...
double *gdf[6],*x,*y,*xc,*yc,*fllr[5],*fldu[5],
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   for(i=2*IT/3; i < IT-2; i++)
    for(k=0; k <5; k++)
     gdf[k][IC(i,JT-2)]=gdf[k][IC(i,JT-1)]=gdf[k][IC(i,JT-3)];
...
// Eu_flux
     fllr[0][Ilr(i,j)]=db*ub*nlr[0][Ilr(i,j)]+db*vb*nlr[1][Ilr(i,j)];
     fllr[1][Ilr(i,j)]=(db*ub*ub+pb)*nlr[0][Ilr(i,j)]+db*ub*vb*nlr[1][Ilr(i,j)];
     fllr[2][Ilr(i,j)]=db*ub*vb*nlr[0][Ilr(i,j)]+(db*vb*vb+pb)*nlr[1][Ilr(i,j)];
     fllr[3][Ilr(i,j)]=ub*(e+pb)*nlr[0][Ilr(i,j)]+vb*(e+pb)*nlr[1][Ilr(i,j)];
...
// Eu_flux
     fldu[0][Idu(i,j)]=db*ub*ndu[0][Idu(i,j)]+db*vb*ndu[1][Idu(i,j)];
     fldu[1][Idu(i,j)]=(db*ub*ub+pb)*ndu[0][Idu(i,j)]+db*ub*vb*ndu[1][Idu(i,j)];
     fldu[2][Idu(i,j)]=db*ub*vb*ndu[0][Idu(i,j)]+(db*vb*vb+pb)*ndu[1][Idu(i,j)];
     fldu[3][Idu(i,j)]=ub*(e+pb)*ndu[0][Idu(i,j)]+vb*(e+pb)*ndu[1][Idu(i,j)];
...
void new_gdf(void)
{
 int i,j,n,k;
 double p,e;
 static double rhs[5],a0[5],a1[5],fl[5];
